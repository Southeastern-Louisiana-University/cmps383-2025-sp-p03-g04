...
export default function SeatSelectionScreen() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        await booking.loadSeatingLayout(Number(id), user?.id);
      } catch (error) {
        console.error("Failed to load seat selection data:", error);
        Alert.alert(
          "Error",
          "Failed to load seating information. Please try again."
        );
      }
    };
  ...
  if (booking.isLoading || !booking.seatingLayout || !booking.showtime) {
    return (
  ...
  const sortedRowKeys = Object.keys(booking.seatingLayout.rows).sort();
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            {sortedRowKeys.map((rowKey) => (
              <View key={rowKey} style={styles.row}>
                ...
                <View style={styles.seats}>
                  {booking.seatingLayout?.rows[rowKey]?.map((seat) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          !isTaken && booking.toggleSeatSelection(seat.id)
                        }
                        disabled={isTaken}
                      >
                        <ThemedText
                    ...
                    <View style={styles.ticketTypeButtons}>
                      {["Adult", "Child", "Senior"].map((type) => (
                        <TouchableOpacity
                          ...
                          onPress={() => booking.setTicketType(seatId, type)}
                        >
                          <ThemedText
                            style={[
                              styles.typeButtonText,
                              (booking.ticketTypes[seatId] || "Adult") ===
                ...
                {booking.selectedSeats.length > 0
                  ? booking.selectedSeats
                      .map((seatId) => {
                        for (const rowKey in booking.seatingLayout?.rows ||
                          {}) {
                          const seat = booking.seatingLayout?.rows?.[
                            rowKey
                          ]?.find((s) => s.id === seatId);
          ...
          <TouchableOpacity
            style={[
              ...
              (booking.selectedSeats.length === 0 || booking.isLoading) &&
                styles.disabledButton,
            ]}
            onPress={handleContinue}
            disabled={booking.selectedSeats.length === 0 || booking.isLoading}
          >
            {booking.isLoading ? (
...
const styles = StyleSheet.create({
  ...
  loadingText: { marginTop: 16, color: "#9BA1A6" },
  scrollContent: { padding: 16, paddingBottom: 40 },
  movieInfoContainer: { marginBottom: 20 },
  movieTitle: { fontSize: 24, fontWeight: "bold", marginBottom: 8 },
  showtimeInfo: { fontSize: 16, color: "#B4D335", marginBottom: 4 },
  theaterInfo: { fontSize: 14, color: "#9BA1A6" },
  seatMapContainer: { alignItems: "center", marginVertical: 20 },
  screen: {
    width: "80%",
    height: 20,
  ...
  screenText: { color: "#FFFFFF", fontSize: 12 },
  row: { flexDirection: "row", marginBottom: 10, alignItems: "center" },
  rowLabel: { width: 30, alignItems: "center", justifyContent: "center" },
  rowLabelText: { fontSize: 14, fontWeight: "500" },
  seats: { flexDirection: "row" },
  seat: {
    width: 30,
    height: 30,
    borderRadius: 6,
  ...
  selectedSeat: { backgroundColor: "#B4D335" },
  takenSeat: { backgroundColor: "#666666" },
  seatText: { color: "#FFFFFF", fontSize: 12, fontWeight: "500" },
  selectedSeatText: { color: "#242424" },
  takenSeatText: { color: "#999999" },
  legendContainer: {
    flexDirection: "row",
    justifyContent: "center",
    marginTop: 20,
  ...
  availableSeat: { backgroundColor: "#1E3A55" },
  ...
  seatLabel: { fontSize: 16, width: "30%" },
  ticketTypeButtons: {
    flexDirection: "row",
    width: "70%",
  ...
  selectedTypeButton: { backgroundColor: "#B4D335" },
  typeButtonText: { fontSize: 14 },
  selectedTypeText: { color: "#1E2429", fontWeight: "bold" },
  summaryContainer: {
    backgroundColor: "#262D33",
    borderRadius: 12,
    padding: 16,
  ...
  totalPrice: { fontSize: 18, color: "#B4D335", fontWeight: "bold" },
  continueButton: {
  ...
  disabledButton: { backgroundColor: "#4A4A4A", opacity: 0.7 },
  continueButtonText: { color: "#1E2429", fontSize: 16, fontWeight: "bold" },
});
